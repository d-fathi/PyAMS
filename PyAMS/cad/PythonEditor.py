#-------------------------------------------------------------------------------
# Name:        Editor of Spice(cmd) and Html
# Author:      D.fathi
# Created:     08/06/2022
# Update:      04/11/2024
...
def listOfModels(self,modelName,directory):
   result=[];
   notSubckt= True;
   ...
   if(file_exists):
           ...
           for i in range(len(t)):
              ...
              if(len(k)>1):
                if((k[0].lower()=='.model') and notSubckt):
                   #print(k[1])
                   result+=[k[1]]
                if(k[0].lower()=='.subckt'):
                   #print(k[1])
...
class openCodePyCode:
    ...
    def __init__(self,file):
        ...
        self.highlight = syntax_pars.PythonHighlighterPyCode(self.editor.document())
        self.w.resize(1150, 840)
        self.layout = QtWidgets.QVBoxLayout(self.w)
        self.layout.addWidget(self.editor);
        self.editor.toPlainText().encode("utf-8")
...
def showCodeSpice(self,codeSpice):
    dialog =openCodeSpice(codeSpice);
    dialog.w.setWindowTitle("Code  Spice");
    dialog.w.setWindowIcon(self.setWin.setIcon);
    dialog.setWin=self.setWin;
    if dialog.w.exec():
        self.setWin.ui.m_webview.page().runJavaScript("setCodeSpice("+str([dialog.editor.toPlainText()])+");");
